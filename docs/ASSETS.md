...
| nacos | `github.com/jninng/nacos` | 原生 | `nacos`（config/registrar 子节分立） | 启动期配置（不热更） | 可用 | cfg 配置源（Source 签名直传 Attach）+ reg 服务注册（标准生命周期），v0.1.0（go 1.23），接入见 DESIGN.md 附录 B |
