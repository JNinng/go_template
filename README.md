...
3. `go build ./... && go test ./...`
4. 按需引入组件：内置组件直接 import（`internal/components/`，轻量样例）；组件资产 `go get` + 在装配入口接线（远程源 → `internal/app/sources.go` 的 `setupSources`；业务组件 → **业务入口 `internal/app/biz.go`**，`AddComponent` 或等价手写展开）+ 粘贴配置节
